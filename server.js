...
async function buildDashboardData() {
  ...
  // Group meetings by scheduled date (when the call actually happens)
  const bookingsByDateRep = {}; // dateStr -> repName -> count
  const bookingsByDate    = {}; // dateStr -> total count

  for (const m of meetings) {
    const bDate = m.scheduledStr;
  ...
  // If a rep has 0 meetings scheduled on 2+ of those days where team total > 0 → ABSENT.
  // 1 absent day → NO-CALL DAY. 0 absent days → ACTIVE.
  // If rep has any meetings scheduled today, they are active regardless of prior pattern.
  ...
  for (const repName of outboundRepNames) {
    ...
    // If rep has meetings scheduled today or in the future, they are active
    const hasFutureMeetings = meetings.some(m => m.rep === repName && m.scheduledStr >= todayStr);
  ...
  // teamAvgPerDay: meetings scheduled across last 3 completed biz days
